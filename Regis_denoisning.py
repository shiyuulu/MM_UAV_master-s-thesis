...
from lap_batch import sample_pcd, fine_reg, calculate_errors, merge_point_clouds
...
def draw_registration_result(source, target, transformation):
    '''
    visualization of registration result
    '''
...
def find_corr(xyz0, xyz1, F0, F1, subsample_size=-1):
    '''
    find corresboundings based on nearest neighbors
    '''
...
def alignment(config):
    '''
    Core part of performing registration using FCGF
    input:config
    return:    
...
def write_pcd(pcd_1,pcd_2,folder,f1_name,f2_name):
    '''
    write point cloud to given path
    '''
...
def simplify(las_source,las_target,cons,cont,stable_ob,  radius=0.3 ):
    '''
    simplification and denoising of point cloud 

    las_souce: LAS source point cloud 
    las_target: LAS target point cloud
    conu: constant of source
    cont: constant of target
    stabel_ob: selected constant number
    radius: searching radius of graph

    '''

    source_stabel_ind=list(np.where(np.in1d(cons,stable_ob))[0])
    target_stabel_ind=list(np.where(np.in1d(cont,stable_ob))[0])
...
def FCGF_regis(source, target,transformation_matrix, subfolder_path, GT):
    ...
    '''
    coarse registration based on FCGF
    '''
...
def coarse_reg(uav_simplify,mm_simplify,subfolder_path,transformation_matrix,GT):
    '''
    perform coarse registration

    uav_simplify: simiplified constructed surface class in UAV
    mm_simplify: simiplified constructed surface class in MM

    sbfolder_path: path to results folder
    transformation_matrix: random transformation
...
def Simplify_selected_class(lazu,lazm,conu,conm,ob_list1,ob_list2, subfolder_path,radius, gamma ):
    '''
    perform simplification to selected point cloud based on graph

    lazu:UAV point cloud
    lazm:MM point cloud
    conu:Constant of UAV
    conm:Constant of MM
    ob_list1:constant numbers of building
    ob_list2:constant numbers of surfaces

    subfolder_path:folder path of results 

    radius: parameter for construction graph
    '''
    ...
    uav_merge=merge_point_clouds(uav_b,uav_s)
    uav_merge_simplify=merge_point_clouds(uav_simplify_b,uav_simplify_s)
    mm_merge=merge_point_clouds(mm_b,mm_s)
    mm_merge_simplify=merge_point_clouds(mm_simplify_b,mm_simplify_s)
...
def main():
    ...
    for file_name in os.listdir(path):
        ...
        if file_name.endswith('.las'):
            ...
            trans_sub=coarse_reg( uav_sub_simplify, mm_sub_simplify, subfolder_path,transformation_matrix,GT)
            trans.append(trans_sub)

            trans_ini=np.dot(trans_sub,transformation_matrix)

            t_icp=fine_reg( oriu, orim, trans_ini, GT)
